...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.avatar=this.avatar.substring(0,this.avatar.length-4)
      /*获取全部动态*/
      ...
      getManuscript:function(id){
        if(id!=null){
          ...
          axios.post(url).then(res=>{
            ...
            this.manuInfo.cover=this.manuInfo.cover.substring(0,this.manuInfo.cover.length-4)
            let url1 =`http://localhost:8088/user/selById/${this.manuInfo.userId}`
            axios.post(url1).then(res=>{
              this.userInfo=res.data
              if(this.userInfo.picture!=null)
                this.userInfo.picture=this.userInfo.picture.substring(0,this.userInfo.picture.length-4)
            })
            return true
          })
        }
